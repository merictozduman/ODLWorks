...
var_domain_lower:=LOWER(
                 substr(vEmailStep2,
                 instr(vEmailStep2, '@') + 1,
                 instr(vEmailStep2, '.', instr(vEmailStep2, '@')) - instr(vEmailStep2, '@') - 1) 
                      );
...
BEGIN	
	--3-exp_domain step'ine aittir.
     ...
     exp_email_subdomain :=FNC_REVERSE(SUBSTR(var_email_reverse,1,INSTR(var_email_reverse,'.')-1)); --subdomain tespiti yanlış. hotmail.com da com u veriyor
